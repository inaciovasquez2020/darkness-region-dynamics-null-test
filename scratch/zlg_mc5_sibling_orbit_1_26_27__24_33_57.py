#!/usr/bin/env python3
"""Fail-closed verifier for size-7 sibling orbit ((1,26,27),(24,33,57))."""
from itertools import product

PAIR = ((1, 26, 27), (24, 33, 57))
...
def unsimplified(vals):
    ...
    # Pi1=<x1,x2+x3+x4>, Pi2=<x3+x4,x1+a2>.
...
def simplified(vals):
    ...
    return (
        ...
        ^ ((m1 ^ sigma ^ tau) & x1)
        ^ ((l1 ^ rho) & x2)
        ^ ((rho ^ m2 ^ (l2 & m2) ^ ups ^ (tau & m2)) & x3)
        ^ ((rho ^ l2 ^ (l2 & m2) ^ ups ^ (tau & l2)) & x4)
        ^ (l1 & m1)
        ^ (rho & sigma)
        ^ (tau & l2 & m2)
        ^ (tau & ups)
...
def main():
    ...
    print(
        ...
        "pair=((1,26,27),(24,33,57)) orbit_size=2 "
    ...
    print("LEVEL5_SIBLING_SIZE7_ORBIT_1_26_27__24_33_57_CLOSED")
